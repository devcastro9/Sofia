...
''     End If
  ...
  VAR_COD2 = Ado_datos.Recordset!compra_codigo
     ...
     sino = MsgBox("Para Aprobar TODOS los registro elija SI, para Aprobar sólo el registro elegido elija NO ... ", vbYesNo + vbQuestion, "Atención")
     If sino = vbYes Then
...
'    End If
    If Ado_detalle1.Recordset.RecordCount > 0 Then
        'sino = MsgBox("Desea APROBAR el Registro ? (Ya no podrá modificarlo...)", vbYesNo + vbInformation, "Atención")
        sino = MsgBox("Desea APROBAR el Ingreso a Almacen ? (Ya no podrá modificar este registro...)", vbYesNo + vbInformation, "Atención")
        If sino = vbYes Then
            DETALLE2 = Ado_detalle2.Recordset!adjudica_codigo
            VAR_COD2 = Ado_datos.Recordset!compra_codigo
...
Private Sub BtnAprobar3_Click()
    If glusuario = "JYMAMANI" Or glusuario = "RTORREZ" Or glusuario = "CNUÑEZ" Or glusuario = "SLIMACHI" Or glusuario = "RCUELA" Or glusuario = "ADMIN" Then
        If Ado_detalle2.Recordset.RecordCount = 0 Then
            MsgBox "No se puede APROBAR, porque Debe registrar previamente la Factura...!! ", vbExclamation
            Exit Sub
        End If
