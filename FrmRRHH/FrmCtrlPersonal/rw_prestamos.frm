...
Dim FECHA_ABC As Date
Dim msgSalir, ado, modif, modif_det As String
Dim queryinicial, mes_aux  As String
...
''                rs_aux2.O  pen SQL_FOR, db, adOpenKeyset, adLockOptimistic
...
dia = 1
FECHA_ABC = dia & "/" & mes_grupo & "/" & gestion
mes_aux = DateSerial(Year(FECHA_ABC), Month(FECHA_ABC) + 1, 0)

If CUOTA = 0 Then
rs_datos1!cobranza_fecha_prog = CDate(mes_aux)
Else
rs_datos1!cobranza_fecha_prog = CDate(mes_aux)
End If
rs_datos!fecha_fin = DateAdd("m", CUOTA, CDate(rs_datos1!cobranza_fecha_prog))
CUOTA = CUOTA + 1
