...
Private Sub cmdOk_Click()
 ...
 Dim NoDias, NoHoras, NoMin As Double
...
'     rw_ficha_rrhh.Ado_Memo.Recordset!ARCHIVO_MEM = Trim(rw_ficha_rrhh.Ado_datos.Recordset!iniciales) & "_Memos_" & rw_ficha_rrhh.Ado_Memo.Recordset!CORREL & ".pdf"
      ...
      rw_ficha_rrhh.Ado_Memo.Recordset("codigo_empresa").Value = rw_ficha_rrhh.Ado_datos.Recordset!codigo_empresa
      If optSi.Value = True Then
...
Private Sub Dtc_ParDes_Click(Area As Integer)
    ...
    dtc_estado.BoundText = Dtc_ParDes.BoundText
     ...
     If Dtc_Par.Text = "ANT" Then
    
    cbo_dias.Enabled = False
    optSi.Value = True
    lblLabels(9).Caption = "Monto Anticipo          Observaciones"
    Else
    
    cbo_dias.Enabled = True
    optNo.Value = True
    lblLabels(9).Caption = "Monto Sanción          Observaciones"
    End If
End Sub
