...
'  cmdGraba3.Enabled = False
  'trv.Enabled = True
    ...
    Set rs_CtrlAsistencia = New ADODB.Recordset
    rs_CtrlAsistencia.Open "select * from mc_uso_suelo ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_CtrlAsistencia.Recordset = rs_CtrlAsistencia
'    Dtc_Establ_Des.BoundText = Dtc_Establ.BoundText

    Set rs_CtrlPermiso = New ADODB.Recordset
    rs_CtrlPermiso.Open "select * from mc_sector  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_CtrlPermiso.Recordset = rs_CtrlPermiso
        
    Set rs_Form = New ADODB.Recordset
    rs_Form.Open "select * from gc_Tipo_Tramite ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Form.Recordset = rs_Form
    ...
    rs_comunid.Open "select * from gc_comunidad where estado_registro ='S' ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Comunid.Recordset = rs_comunid
'    Dtc_local.BoundText = Dtc_local_cod.BoundText

    Set rs_comunidV = New ADODB.Recordset
    rs_comunidV.Open "select * from gv_comunidad where estado_registro ='S' ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_ComunidV.Recordset = rs_comunidV

    Set rs_persona = New ADODB.Recordset
    rs_persona.Open "select * from gc_beneficiario where estado_registro ='S' and tipo_beneficiario ='1' ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_persona.Recordset = rs_persona
'    Dtc_pers_1apell.BoundText = Dtc_pers_id.BoundText
'    Dtc_pers_2Apell.BoundText = Dtc_pers_id.BoundText
'    Dtc_Pers_nombre.BoundText = Dtc_pers_id.BoundText

    Set rs_consultor = New ADODB.Recordset
    rs_consultor.Open "select * from gc_beneficiario where estado_registro ='S' and tipo_beneficiario ='2' ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_consultor.Recordset = rs_consultor
'    Dtc_pac_1apell.BoundText = Dtc_Pac_Id.BoundText
'    Dtc_pac_2apell.BoundText = Dtc_Pac_Id.BoundText
'    Dtc_pac_nombre.BoundText = Dtc_Pac_Id.BoundText
'    Dtc_Genero.BoundText = Dtc_Pac_Id.BoundText

    Set rs_Empresa = New ADODB.Recordset
    rs_Empresa.Open "select * from gc_beneficiario where estado_registro ='S' and tipo_beneficiario ='6' ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Empresa.Recordset = rs_Empresa
    'Dtc_RedSalud_des.BoundText = Dtc_RedSalud.BoundText

    
'    Set rs_ocupac = New ADODB.Recordset
'    rs_ocupac.Open "select * from gc_ocupacion where estado_registro ='S' ", db, adOpenKeyset, adLockOptimistic
'    Set Ado_Ocupac.Recordset = rs_ocupac

    Set rs_ProyUbic = New ADODB.Recordset
    rs_ProyUbic.Open "select * from mo_proy_Id_Ubicacion ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_ProyUbic.Recordset = rs_ProyUbic
    'Set DBGProyUbic.DataSource = Ado_ProyUbic.Recordset

    Set rs_topografia = New ADODB.Recordset
    rs_topografia.Open "select * from mc_Ecologia_topografia ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Topografia.Recordset = rs_topografia

    Set rc_napa_freatica = New ADODB.Recordset
    rc_napa_freatica.Open "select * from mc_napa_freatica  ", DB, adOpenKeyset, adLockOptimistic
    Set ado_napa_freatica.Recordset = rc_napa_freatica

    Set rs_Agua_Calidad = New ADODB.Recordset
    rs_Agua_Calidad.Open "select * from mc_agua_calidad  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Agua_Calidad.Recordset = rs_Agua_Calidad

    Set rs_vegetacion = New ADODB.Recordset
    rs_vegetacion.Open "select * from mc_Ecologia_vegetacion  ", DB, adOpenKeyset, adLockOptimistic
    Set ado_vegetacion.Recordset = rs_vegetacion

    Set rs_Red_Drenaje = New ADODB.Recordset
    rs_Red_Drenaje.Open "select * from mc_red_drenaje  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Red_Drenaje.Recordset = rs_Red_Drenaje

    Set rs_subsector = New ADODB.Recordset
    rs_subsector.Open "select * from mc_subsector  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_SubSector.Recordset = rs_subsector

    Set rs_Sector_Act = New ADODB.Recordset
    rs_Sector_Act.Open "select * from mc_sector_actividad  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_SectorAct.Recordset = rs_Sector_Act
'    Dtc_Act_Des.BoundText = Dtc_Act.BoundText
    
    Set rs_ProyNat = New ADODB.Recordset
    rs_ProyNat.Open "select * from mc_Proy_Naturaleza  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_ProyNat.Recordset = rs_ProyNat
    
    Set rs_ProyAmbito = New ADODB.Recordset
    rs_ProyAmbito.Open "select * from mc_Proy_Ambito  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_ProyAmbito.Recordset = rs_ProyAmbito
    
    Set rs_Proy_tipo = New ADODB.Recordset
    rs_Proy_tipo.Open "select * from mc_proy_tipo  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Proy_tipo.Recordset = rs_Proy_tipo
    
    Set rs_Proyecto = New ADODB.Recordset
    rs_Proyecto.Open "select * from mc_Proyectos  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_Proyecto.Recordset = rs_Proyecto
    
    Set rs_ProyFase = New ADODB.Recordset
    rs_ProyFase.Open "select * from mc_proy_fase  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_ProyFase.Recordset = rs_ProyFase
    
    Set RS_ProyEtapa = New ADODB.Recordset
    RS_ProyEtapa.Open "select * from mc_Proy_Etapa  ", DB, adOpenKeyset, adLockOptimistic
    Set Ado_ProyEtapa.Recordset = RS_ProyEtapa
    
    Set rs_UniMed = New ADODB.Recordset
    rs_UniMed.Open "select * from Al_UnidadMedida WHERE estatus = 'B' ", DB, adOpenKeyset, adLockOptimistic
    Set AdoUniMed.Recordset = rs_UniMed

'    Set rs_UnidadMedida = New ADODB.Recordset
'    If rs_UnidadMedida.State = 1 Then rs_UnidadMedida.Close
'    rs_UnidadMedida.Open "select * from Al_UnidadMedida WHERE (estatus = 'A') ", db, adOpenKeyset, adLockReadOnly
'    Set Ado_UnidadMedida.Recordset = rs_UnidadMedida
'    Ado_UnidadMedida.Refresh
