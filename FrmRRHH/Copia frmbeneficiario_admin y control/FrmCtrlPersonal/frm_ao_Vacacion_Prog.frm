...
Begin VB.Form frm_ao_Vacacion_Prog 
   ...
   Begin VB.Frame FraProyecto 
      ...
      Begin VB.ComboBox TxtGestion 
         ...
         Left            =   1320
         List            =   "frm_ao_Vacacion_Prog.frx":D704E
         TabIndex        =   19
         Top             =   840
         Width           =   900
      End
      ...
      Begin VB.ComboBox txt01 
         Height          =   315
         ItemData        =   "frm_ao_Vacacion_Prog.frx":D70D6
         Left            =   3120
         List            =   "frm_ao_Vacacion_Prog.frx":D70FE
         TabIndex        =   1
         Text            =   "ENERO"
         Top             =   840
         Width           =   2295
      ...
      Begin VB.Label lblLabels 
         ...
         BackColor       =   &H8000000B&
         BackStyle       =   0  'Transparent
         Caption         =   $"frm_ao_Vacacion_Prog.frx":D7167
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   9.75
            Charset         =   0
...
If sel = 1 Then
       ...
       rs_aux6.Open "SELECT * FROM ro_personal_contratado WHERE estado_codigo <> 'ANL' AND beneficiario_codigo = '" & txtBenef.Text & "'", db, adOpenStatic 'order by beneficiario_denominacion
      ...
      'ProgressBar1.Max =
       ...
       rs_aux5.Open "select * from ro_vacaciones_programadas where ges_gestion = '" & TxtGestion.Text & "' AND beneficiario_codigo = '" & txtBenef.Text & "'", db, adOpenKeyset, adLockOptimistic, adCmdText
       ...
       If rs_aux5.RecordCount = 0 Then
        ...
        rw_ficha_rrhh.Ado_VacacionesProg.Recordset("Hora_reincorporacion").Value = "08:00:00"
         ...
         Set rs_correlativo = New ADODB.Recordset
         rs_correlativo.Open "select * from ro_vacaciones_programadas WHERE beneficiario_codigo = '" & txtBenef.Text & "'  ", db, adOpenKeyset, adLockOptimistic
        If rs_correlativo.RecordCount > 0 Then
                    rw_ficha_rrhh.Ado_VacacionesProg.Recordset!CORREL = rs_correlativo.RecordCount
              Else
                    rw_ficha_rrhh.Ado_VacacionesProg.Recordset!CORREL = 1
...
Else
       rw_ficha_rrhh.ProgressBar1.Visible = True
       If rs_aux6.State = 1 Then rs_aux6.Close
       rs_aux6.Open "SELECT * FROM ro_personal_contratado WHERE estado_codigo <> 'ANL'", db, adOpenStatic 'order by beneficiario_denominacion
      ...
      'ProgressBar1.Max =
       ...
       rw_ficha_rrhh.ProgressBar1.Value = rw_ficha_rrhh.ProgressBar1.Value + 1
       If rs_aux5.State = 1 Then rs_aux5.Close
       rs_aux5.Open "select * from ro_vacaciones_programadas where ges_gestion = '" & TxtGestion.Text & "' AND beneficiario_codigo = '" & rs_aux6!beneficiario_codigo & "'", db, adOpenKeyset, adLockOptimistic, adCmdText
        VAR_GES = DateDiff("yyyy", rs_aux6!fecha_ingreso, Date)
       DIA_IN = Day(rs_aux6!fecha_ingreso)
       MES_IN = Month(rs_aux6!fecha_ingreso)
       ANO_IN = Year(rs_aux6!fecha_ingreso)
       ...
       Else
        If rw_ficha_rrhh.Ado_VacacionesProg.Recordset("estado_codigo") = "REG" Or glusuario = "VPAREDES" Then
            db.Execute "UPDATE ro_vacaciones_programadas SET dias_Programados = " & txt_dias_vac.Text & ", Dias_Pendientes = " & Val(txt_dias_vac.Text) - rs_aux5!dias_utilizados & " , mes_control = '" & Txt01.Text & "' where beneficiario_codigo = '" & rs_aux6!beneficiario_codigo & "' and ges_gestion = '" & TxtGestion.Text & "'"
        End If
       End If
       rs_aux6.MoveNext
      Wend
