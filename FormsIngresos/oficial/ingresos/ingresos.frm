...
Begin VB.Form FrmIngresosabm 
   ...
   Begin VB.Frame FraIngresosDat 
      ...
      Begin MSAdodcLib.Adodc AdoFc_beneficiario 
         ...
         _Version        =   393216
      End
      Begin MSDataListLib.DataCombo DtCDenominacion_Convenio 
         ...
         Text            =   ""
      End
      Begin MSDataListLib.DataCombo DtCcodigo_convenio 
         Bindings        =   "ingresos.frx":3800
         Height          =   315
         Left            =   60
         TabIndex        =   74
         ToolTipText     =   "Elije el Código de Organismo de Financiamiento"
         ...
         Text            =   ""
      End
      Begin MSAdodcLib.Adodc AdoFc_convenios 
         Height          =   330
         Left            =   1740
         Top             =   4140
         Visible         =   0   'False
...
'                DtCDenominacion_tipo.Enabled = False
            ...
            DtCcodigo_convenio.Text = IIf(IsNull(AdoIngresos.Recordset("codigo_convenio")) = True, "", AdoIngresos.Recordset("codigo_convenio"))
            ...
            If AdoIngresos.Recordset("Codigo_tipo") = "REC" Or AdoIngresos.Recordset("Codigo_tipo") = "DYR" Or AdoIngresos.Recordset("Codigo_tipo") = "ANI" Or AdoIngresos.Recordset("Codigo_tipo") = "DVI" Then
...
'===== Proceso para Añadir y/o modificar Datos
    ...
    If swcopiar = 1 Then
      ...
      If V_accion = "ANI" Or V_accion = "DES" Or V_accion = "DVI" Then
        ...
        'Txtmonto_dolares = Round(((TxtMonto_bolivianos / GlTipoCambioOficial) * -1), 2) gvi 28/06/01
        txtmonto_Dolares = txtmonto_Dolares * -1
        'TxtMonto_bolivianos = Round((TxtMonto_bolivianos * -1), 2) gvi 28/06/01
...
'            DtCOrg_codigo.Enabled = True
          Select Case V_accion
            Case "REC"
              ...
              Me.DtCcodigo_beneficiario.Visible = False
              Me.DtCdenominacion_beneficiario.Visible = False
...
'**** aqui consultar tia que hacer ***************
    ...
    If AdoIngresos.Recordset("codigo_tipo") = "DVI" Then
      j = 1
      v_Tipo_Comp(1, 1) = "DVI"
    End If
...
' GVI 02/07/01 VERIFICAR SI SE ESTA ABRIENDO BIEN
        If rstdestino.State = 1 Then rstdestino.Close
        rstdestino.Open "select * from fc_relacionador_ingresos where inst = 'DEV' and rec_rub_i <= " & (DtCrbr_codigo) & " and rec_rub_f >= " & (DtCrbr_codigo), db, adOpenKeyset, adLockReadOnly
        If rstdestino2.State = 1 Then rstdestino2.Close
        rstdestino2.Open "select * from fc_relacionador_ingresos where inst = 'EFE' and rec_rub_i <= " & (DtCrbr_codigo) & " and rec_rub_f >= " & (DtCrbr_codigo) & " and subcta_deb2 = '" & IIf(fte_codigo1 = "41", "01", IIf(fte_codigo1 = "43", "02", IIf(fte_codigo1 = "80", "03", ""))) & "' ", db, adOpenKeyset, adLockReadOnly
...
'========================================
    '==== verifica ya fue contabilizado
      yacontabilizo = 0
      If rstdestino.State = 1 Then rstdestino.Close
      rstdestino.Open "select * from co_comprobante_m where Cod_trans = '" & AdoIngresos.Recordset!correlativo_anterior & "' and org_codigo = '" & AdoIngresos.Recordset!org_codigo & "' and tipo_comp = '" & v_Tipo_Comp(1, i) & "' AND STATUS = 'S'", db, adOpenKeyset, adLockOptimistic
      If rstdestino.RecordCount > 0 Then
        yacontabilizo = 1
      Else
        yacontabilizo = 0
...
'anterior
      ...
      rstdestino!tipo_moneda = AdoIngresos.Recordset!tipo_moneda
...
Private Sub CmdImprimir_Click()
  If rstIngresos.RecordCount > 0 Then
    ...
    Call prt_cmbteIng(AdoIngresos.Recordset!ges_gestion, AdoIngresos.Recordset!org_codigo, AdoIngresos.Recordset!correlativo_ingreso)
  Else
...
Private Sub f_actual_rec(org, codant)
  ...
  rsrecalc.Open "select sum(monto_dolares) as acumDl from fo_ingresos where org_codigo = '" & org & "' and  correlativo_anterior = '" & codant & "' and codigo_tipo = 'REC' and estado_recaudado= 'S'", db, adOpenKeyset, adLockReadOnly
  ...
  End If
  If rsrecalc.State = 1 Then rsrecalc.Close
  rsrecalc.Open "select * from fo_ingresos where org_codigo = '" & org & "' and correlativo_ingreso = '" & codant & "' ", db, adOpenKeyset, adLockOptimistic
  If rsrecalc.RecordCount > 0 Then
