...
Begin VB.Form mw_ventas_cabecera 
   ...
   MDIChild        =   -1  'True
   Moveable        =   0   'False
   ScaleHeight     =   8.27186e6
   ScaleMode       =   0  'User
   ScaleWidth      =   4.73507e9
...
Dim VAR_UNIDAD0, VAR_UNI_ACTUAL As String
...
If (Not Ado_datos.Recordset.BOF) And (Not Ado_datos.Recordset.EOF) Then
   If Not IsNull(Ado_datos.Recordset!venta_codigo) Then
        ...
        VAR_UNI_ACTUAL = Ado_datos.Recordset!unidad_codigo
        lbl_cerrado.Caption = ""
...
Private Sub BtnAprobar_Click()
    ...
    rs_aux11.Open "Select * from mv_bienes_vs_venta_det WHERE venta_codigo = " & NumComp & "  ", db, adOpenStatic
    ...
    rs_aux12.Open "Select * from ao_ventas_detalle WHERE venta_codigo = " & NumComp & " and par_codigo=''  ", db, adOpenStatic
    ...
    rs_aux21.Open "Select SUM(cobranza_programada_dol) AS SumaCuota from ao_ventas_cobranza_prog WHERE venta_codigo = " & NumComp & " AND es_liquidacion = 'NO' ", db, adOpenStatic
    If rs_aux21.RecordCount > 0 Then
        If Round(VAR_DOL2, 0) = Round(rs_aux21!SumaCuota, 0) Then        'Las Cuotas no igualan con el Total del contrato
        Else
            MsgBox "No se puede APROBAR, la SUMA de las Cuotas Dolares, NO iguala con el TOTAL del Contrato Dolares, verifique y vuelva a intentar ...", , "Atención"
            Exit Sub
        End If
    End If
   If IsNull(Ado_datos.Recordset("venta_tipo")) Or Ado_datos.Recordset("venta_tipo") = "" Or (VAR_DOL2 = 0) Or (Ado_datos.Recordset!unidad_codigo_ant = "") Or IsNull(Ado_datos.Recordset!unidad_codigo_ant) Then
   ...
   Else
     If Ado_datos.Recordset("estado_codigo") = "REG" Then
       ...
       If sino = vbYes Then
           ...
           db.Execute "UPDATE ao_ventas_cabecera SET usr_codigo_aprueba = '" & glusuario & "' WHERE venta_codigo = " & correlv & " "
          ...
          Else                             'UNA CUOTA PARA UNA FACTURA
            ...
            Set rs_idfactura = New ADODB.Recordset
                ...
                Set rs_idfactura = db.Execute("INSERT INTO ao_ventas_cobranza_fac (ges_gestion, venta_codigo, doc_codigo_fac,              beneficiario_codigo_fac,                                beneficiario_nit,           glosa_Descripcion,                                  beneficiario_RazonSocial, nro_dui,      total_bs,                                       total_dol,                                      cambio_oficial, " & _
                        ...
                        " VALUES ('" & glGestion & "',  " & NumComp & ", '" & VAR_DOCFAC & "', '" & Ado_datos16.Recordset!beneficiario_codigo & "', '" & dtc_codigo2A.Text & "', '" & Ado_datos16.Recordset!cobranza_concepto_plazo & "', '" & dtc_desc2A.Text & "',  '0', " & Ado_datos16.Recordset!cobranza_programada_bs & ",  " & Ado_datos16.Recordset!cobranza_programada_dol & ",  " & GlTipoCambioOficial & ",  " & _
...
Private Sub BtnAddDetalle2_Click()
  ...
  '  End If
    If (glusuario = "CPAREDES" Or glusuario = "ADMIN" Or glusuario = "GSOLIZ" Or glusuario = "ASANTIVAÑEZ" Or glusuario = "CPLATA" Or glusuario = "DTERCEROS" Or glusuario = "MARTEAGA" Or glusuario = "RGIL" Or glusuario = "LMORALES" Or glusuario = "GMORA" Or glusuario = "PMAJLUF" Or glusuario = "CSALINAS") Or Ado_datos.Recordset!venta_saldo_p_cobrar_bs > 0 Then             'Or glusuario = "ADMIN"
...
Private Sub CmdGrabaCobro_Click()
   If Ado_datos.Recordset!venta_tipo = "V" And Txt_liquida.Text = "SI" Then
    MsgBox "NO se puede realizar una LIQUIDACION, en un contrato de FACTURACION LOCAL.. !! Vuelva a Intentar ...", vbExclamation, "Atención"
    Exit Sub
   End If
    'dtc_codigo11.Text ="V"
    ...
    Else
        If swnuevo = 1 Then
            If (rs_aux3!totbs2) + CDbl(TxtMonto) > Ado_datos.Recordset!venta_monto_total_bs Then
                If (glusuario = "NROMERO" Or glusuario = "GSOLIZ" Or glusuario = "CPAREDES" Or glusuario = "RGIL" Or glusuario = "LMORALES" Or glusuario = "GMORA" Or glusuario = "PMAJLUF" Or glusuario = "DTERCEROS" Or glusuario = "CPLATA" Or glusuario = "ADMIN" Or glusuario = "CSALINAS") Then
        ...
        Else
            If (rs_aux3!totbs2) - VAR_MBS2 + CDbl(TxtMonto) > Ado_datos.Recordset!venta_monto_total_bs Then
                If (glusuario = "MARTEAGA" Or glusuario = "GSOLIZ" Or glusuario = "RGIL" Or glusuario = "LMORALES" Or glusuario = "GMORA" Or glusuario = "PMAJLUF" Or glusuario = "DTERCEROS" Or glusuario = "CPLATA" Or glusuario = "ADMIN" Or glusuario = "CSALINAS") Then
                    MsgBox "ADVERTENCIA, el Monto acumulado de cobranzas <" + lbl_monto.Caption + "> sobrepasaran <" + lbl_totalBs.Caption + "> . Solo en Caso de Liquidaciones ...", vbExclamation, "Atención"
                Else
...
'  Set rstdestino = New ADODB.Recordset
'  If rstdestino.State = 1 Then rstdestino.Close
'  rstdestino.Open "select * from ao_ventas_cabecera where ges_gestion = '" & gestion0 & "' and venta_codigo = " & NumComp, db, adOpenKeyset, adLockOptimistic
'  If rstdestino.RecordCount > 0 Then
...
Private Sub ABRIR_TABLA_DET()
    Set rs_datos14 = New ADODB.Recordset
        If rs_datos14.State = 1 Then rs_datos14.Close
        rs_datos14.Open "select * from ao_ventas_detalle where venta_codigo = " & NumComp & "  ", db, adOpenKeyset, adLockOptimistic
        ...
        rs_datos16.Open "select * from ao_ventas_cobranza_prog where venta_codigo = " & NumComp & "  ", db, adOpenKeyset, adLockOptimistic
        ...
        Set rs_datos6 = New ADODB.Recordset
        If rs_datos6.State = 1 Then rs_datos6.Close
        rs_datos6.Open "select * from ao_ventas_alcance where venta_codigo= " & NumComp & "  order by ORDEN ", db, adOpenKeyset, adLockOptimistic, adCmdText
  ...
  '===== Proceso para filtrado general de datos(registros no aprobados)
    ...
    Set rs_datos = New Recordset
     ...
     Select Case VAR_DA
        ...
        Case "1.4"    ' ADMIN
            If glusuario = "ADMIN" Or glusuario = "VPAREDES" Or glusuario = "RVALDIVIEZOR" Or glusuario = "CSALINAS" Or glusuario = "CCRUZ" Or glusuario = "LNAVA" Then
                If VAR_UORIGEN = "DVTA" Then
  ...
  '===== Proceso para filtrado general de datos (todos los registros )
    ...
    If VAR_UORIGEN = "DVTA" Then
        If glusuario = "ADMIN" Or glusuario = "GSOLIZ" Or glusuario = "CPLATA" Or glusuario = "RCUELA" Or glusuario = "DTERCEROS" Or glusuario = "CCRUZ" Then
            queryinicial = "select * From av_ventas_cabecera WHERE ((unidad_codigo LIKE '%" + VAR_UNIDAD0 + "%' OR unidad_codigo = 'DVTA' ) AND (estado_codigo <>'ERR')) "
            VAR_SW = 1
        End If
    Else
        If glusuario = "ADMIN" Or glusuario = "JSAAVEDRA" Or glusuario = "RCUELA" Or glusuario = "OCOLODRO" Or glusuario = "JORAQUENI" Or glusuario = "APALACIOS" Then
    ...
    If VAR_SW = 0 Then
        Select Case VAR_DPTO
            Case "1"
                queryinicial = "Select * from av_ventas_cabecera where (estado_codigo <> 'ANL' AND (unidad_codigo = '" & VAR_UORIGEN & "' OR unidad_codigo = '" & parametro & "') AND (Left(edif_codigo, 1) = '1' OR  Left(edif_codigo, 1) = '5' OR  Left(edif_codigo, 1) = '6' )) "
            Case "2"
                queryinicial = "Select * from av_ventas_cabecera where (estado_codigo <> 'ANL' AND (unidad_codigo = '" & VAR_UORIGEN & "' OR unidad_codigo = '" & parametro & "') AND (Left(edif_codigo, 1) = '2' OR   Left(edif_codigo, 1) = '8' OR   Left(edif_codigo, 1) = '6' OR   Left(edif_codigo, 1) = '5' )) "
            Case "3"
                queryinicial = "Select * from av_ventas_cabecera where (estado_codigo <> 'ANL' AND (unidad_codigo = '" & VAR_UORIGEN & "' OR unidad_codigo = '" & parametro & "') AND (Left(edif_codigo, 1) = '3' OR  Left(edif_codigo, 1) = '4' )) "
            Case "4"
                queryinicial = "Select * from av_ventas_cabecera where (estado_codigo <> 'ANL' AND (unidad_codigo = '" & VAR_UORIGEN & "' OR unidad_codigo = '" & parametro & "') AND Left(edif_codigo, 1) = '4' ) "
            Case "6"
                queryinicial = "Select * from av_ventas_cabecera where (estado_codigo <> 'ANL' AND (unidad_codigo = '" & VAR_UORIGEN & "' OR unidad_codigo = '" & parametro & "') AND Left(edif_codigo, 1) = '6' ) "
            Case "7"
                queryinicial = "Select * from av_ventas_cabecera where (estado_codigo <> 'ANL' AND (unidad_codigo = '" & VAR_UORIGEN & "' OR unidad_codigo = '" & parametro & "') AND (Left(edif_codigo, 1) = '7' OR   Left(edif_codigo, 1) = '8' OR   Left(edif_codigo, 1) = '9' OR   Left(edif_codigo, 1) = '1' )) "
            Case Else
                queryinicial = "Select * from av_ventas_cabecera where (estado_codigo <> 'ANL' AND (unidad_codigo = '" & VAR_UORIGEN & "' OR unidad_codigo = '" & parametro & "') AND Left(edif_codigo, 1) = '" & VAR_DPTO & "' ) "
        End Select
    End If
...
'        Case "1.4", "0"    ' ADMIN
'            If glusuario = "ADMIN" Or glusuario = "VPAREDES" Or glusuario = "RVALDIVIEZOR" Or glusuario = "CSALINAS" Or glusuario = "CCRUZ" Or glusuario = "RCUELA" Then
'                If VAR_UORIGEN = "DVTA" Then
'                    queryinicial = "select * From av_ventas_cabecera WHERE ((unidad_codigo = 'DVTA' OR unidad_codigo = 'DCOMS' OR unidad_codigo = 'DCOMB' OR unidad_codigo = 'DCOMC') AND estado_codigo <> 'ERR') "
...
'            End If
'        Case Else    ' ADMIN
'            If glusuario = "ADMIN" Or glusuario = "VPAREDES" Or glusuario = "RCUELA" Or glusuario = "ASANTIVAÑEZ" Or glusuario = "CSALINAS" Or glusuario = "RCUELA" Then
'                If VAR_UORIGEN = "DVTA" Then
'                    queryinicial = "select * From av_ventas_cabecera WHERE ((unidad_codigo = 'DVTA' OR unidad_codigo = 'DCOMS' OR unidad_codigo = 'DCOMB' OR unidad_codigo = 'DCOMC') AND estado_codigo <> 'ERR' ) "
'                    'queryinicial = "select * From ao_solicitud WHERE estado_codigo = 'REG'  "
'                Else
