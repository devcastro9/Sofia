...
'Set rsCtaBan = New ADODB.Recordset
'If rsCtaBan.State = 1 Then rsCuenta.Close
'rsCtaBan.Open "SELECT * FROM Cns_MovimientoCtaBancaria", db, adOpenKeyset, adLockOptimistic
'If rsCtaBan.RecordCount > 0 Then
    Set rsPCO = New ADODB.Recordset
    If rsPCO.State = 1 Then rsCuenta.Close
    rsPCO.Open "SELECT * FROM co_MovimientoPCO", db, adOpenKeyset, adLockOptimistic
    If rsPCO.RecordCount > 0 Then
        'db.Execute "insert into Cns_MovimientoCtaBancaria (fecha_pago, numero_cheque_trf, denominacion_beneficiario, monto_bolivianos, codigo_pago, monto_dolares, tipo_cambio, cta_descripcion_larga, cta_codigo, org_codigo, chq_trf, fecha_impresion) values ('" & rsPagoDetalle!fecha_pago & "','" & rsPagoDetalle!numero_cheque_trf & "', '" & rsPagoDetalle!denominacion_beneficiario & "', " & rsPagoDetalle!monto_bolivianos & ",'" & rsPagoDetalle!codigo_pago & "'," & rsPagoDetalle!monto_Dolares & ",'" & rsPagoDetalle!tipo_cambio & "','" & rsPagoDetalle!cta_descripcion_larga & "','" & rsPagoDetalle!cta_codigo & "','" & rsPagoDetalle!org_codigo & "', '" & rsPagoDetalle!cheque_o_trf & "', '" & rsPagoDetalle!fecha_impresion_cheque & "' ) "
        db.Execute "insert into Cns_MovimientoCtaBancaria (denominacion_beneficiario) values ('" & rsPCO!codigo_beneficiario & "')"
        rsPCO.MoveNext
    End If
'End If
    'if Cns_MovimientoCtaBancaria
End Sub
