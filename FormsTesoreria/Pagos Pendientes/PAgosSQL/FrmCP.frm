...
Begin VB.Form FrmCP 
   ...
   Begin VB.Frame FraImprimeCmpte 
      ...
      End
   End
   Begin MSComctlLib.ProgressBar PrsBBusca 
      Height          =   312
      Left            =   6420
      TabIndex        =   112
      Top             =   7236
...
Dim i As Variant
                    ...
                    If rspago.RecordCount > 0 Then
                        ...
                        Me.PrsBBusca.Visible = True
                        Me.PrsBBusca.Max = kj + 1
                        While kj <> 0
                        Me.PrsBBusca.Value = kj
                        kj = kj - 1
                        Wend
                        Me.PrsBBusca.Visible = False
...
'Determina si se puede o no cancelar
    ...
    If rsCta.RecordCount > 0 Then
        SaldoBancario_Real = rsCta("Cta_saldo_inicial") - rsCta("Cta_Acumulado") + rsCta("Cta_Saldo_Debe") + rsCta("Cta_Pco_Debe") - rsCta("Cta_Pco_Haber") + rsCta("Cta_Ingresos")
    Else
    ...
    If TxtFechaPago.Text <> "" Then
        AdoPagoDetalle.Recordset("fecha_pago") = TxtFechaPago.Text
...
Dim X As Variant
    ...
    TxtMonto.Enabled = True
    swModifica = 1
    MontoAnterior = AdoPagoDetalle.Recordset("monto_bolivianos")
...
'    'Si se trata de modificar se coloca nuevamente monto_bolivianos en pago_detalle en 0
'
'    Set rsPgDt = New ADODB.Recordset
'    If rsPgDt.State = 1 Then PgDt.Close
'    rsPgDt.Open "select * from pago_detalle where ges_gestion='" & AdoPago.Recordset("ges_gestion") & "' and org_codigo='" & AdoPago.Recordset("org_codigo") & "'and codigo_pago='" & AdoPago.Recordset("codigo_pago") & "'And codigo_pago_detalle = '" & AdoPagoDetalle.Recordset("codigo_pago_detalle") & "'", db, adOpenKeyset, adLockOptimistic
'    If rsPagoDet.RecordCount > 0 Then
'      rsPgDt!monto_bolivianos = 0
'      rsPgDt.Update
'      rsPgDt.Close
'    End If
...
Private Sub CmdRestaurarPagos_Click()
     Set rspago = New ADODB.Recordset
     If rspago.State = 1 Then rspago.Close
    'rsPago.Open "select * from pagos order by codigo_pago", db, adOpenKeyset, adLockOptimistic
    'rspago.Open "select * from pagos where (estado_contabilidad='P' or estado_devengado='S' ) and  estado_aprobacion <>'A' order by codigo_pago", db, adOpenKeyset, adLockOptimistic
    rspago.Open "select * from pagos where (estado_contabilidad='P' or estado_devengado='S') and  estado_aprobacion <>'A' and estado_pagado<>'S' and  estado_compromiso<> 'E' order by codigo_pago", db, adOpenKeyset, adLockOptimistic
    If rspago.RecordCount > 0 Then
      Set AdoPago.Recordset = rspago
      Set DtgPago.DataSource = AdoPago
      DtgPago.ReBind
...
Private Sub Form_Load()
    ...
    'rspago.Open "select * from pagos where (estado_contabilidad='P' or estado_devengado='S') and  estado_aprobacion <>'A' and estado_pagado<>'S' and  estado_compromiso<> 'E' order by codigo_pago", db, adOpenKeyset, adLockOptimistic
    '''rsPago.Open "select * from pagos where (estado_contabilidad='P' or estado_devengado='S' ) order by codigo_pago", db, adOpenKeyset, adLockOptimistic
    '''rsPago.Open "select * from pagos order by codigo_pago", db, adOpenKeyset, adLockOptimistic
    If rspago.RecordCount > 0 Then
      Set AdoPago.Recordset = rspago
      Set DtgPago.DataSource = AdoPago
...
Private Sub OptSeleccion_Click()
    FrmImprimirComprobante.Visible = True
    FraImprimeCmpte.Visible = False
End Sub
