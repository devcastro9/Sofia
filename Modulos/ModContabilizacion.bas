...
'---
  ...
  Dim benef2, benef2_nom As String
  
'Option Explicit
'JQA 24/01/01
Public Sub Cmd_Pagado(P_codigo_pago As Integer, P_codigo_pago_detalle As String, P_org_codigo As String, P_ges_gestion As String)
  Set rstipopy = New ADODB.Recordset
  benef2 = "SIN"
  benef2_nom = "SERVICIO DE IMPUESTOS INTERNOS"
...
'    recSetAuxcomp.Open "SELECT pagos.codigo_convenio, pago_detalle.codigo_Pago,pagos.codigo_solicitud,pago_detalle.codigo_Pago_detalle, Pagos.Fte_Codigo, pagos.Ges_Gestion, Pago_Detalle.Codigo_Beneficiario, pagos.Justificacion, pago_detalle.par_codigo, " & _
'        " pago_detalle.Monto_Total, pago_detalle.Monto_Bolivianos, Pagos.Org_Codigo,Pagos.Codigo_Orden, Pagos.Codigo_Documento, pago_detalle.pro_programa, pago_Detalle.pro_subprograma,pago_Detalle.pro_proyecto,pago_detalle.pro_actividad, pago_detalle.estado_aprobacion, pagos.fecha_aprueba, pagos.Estado_Pagado,Pago_Detalle.Cta_Codigo, " & _
'        " pago_detalle.Monto_Dolares, pago_detalle.Monto_Dolares_DEV, Pago_Detalle.tipo_cambio, Pago_Detalle.tipo_cambio_DEV, pago_detalle.fecha_pago,estado_Devengado, pagos.codigo_UNIDAD, PAGOS.estado_prestamo  FROM PAGOS, PAGO_DETALLE Where pago_detalle.codigo_Pago = pagos.codigo_Pago and pago_detalle.Org_Codigo = pagos.Org_codigo and   " & _
...
'        " pago_detalle.Ges_Gestion = pagos.Ges_Gestion ", db, adOpenDynamic, adLockOptimistic, adCmdText
     recSetAuxcomp.Open "SELECT fv_gastos_y_detalle where  Org_codigo= '" & P_org_codigo & "' and  Ges_Gestion='" & P_ges_gestion & "' and gasto_codigo =" & Val(P_codigo_pago) & " ", db, adOpenDynamic, adLockOptimistic, adCmdText
  ...
  While Not (recSetAuxcomp.EOF)
      ...
      rstipopy.Open "select Pro_proyecto from fc_estructura_programatica where Pro_codigo = " & recSetAuxcomp!pro_codigo & " ", db, adOpenKeyset, adLockReadOnly
      If rstipopy.RecordCount <> 0 Then
          'tipopy = rstipopy!tipo_proyecto
          tipopy = rstipopy!pro_proyecto
      Else
         ' MsgBox "La Categoria Programática elegida no existe"
         MsgBox "Error en la contabilización, No se encontró el Proyecto ", vbExclamation + vbDefaultButton1
         Exit Sub
      End If
      'VERIFICA FUENTE
    ...
    If Sw_Fuente Then
        ...
        recSetAuxActualizar.Open " select * from CO_Comprobante_M  where Cod_Trans='" & P_codigo_pago & "' and Org_Codigo='" & P_org_codigo & "' " & _
          " and Ges_Gestion='" & P_ges_gestion & "' and Tipo_comp='PAC' and Cod_Trans_Detalle='" & recSetAuxcomp!codigo_pago_detalle & _
        ...
        If (recSetAuxActualizar.BOF) And (recSetAuxActualizar.EOF) Then
            ...
            recSetAuxActualizar!estado_prestamo = IIf(IsNull(recSetAuxcomp!estado_prestamo), "N", recSetAuxcomp!estado_prestamo)
            Factura2 = recSetAuxcomp!estado_prestamo
            recSetAuxActualizar!codigo_documento = recSetAuxcomp!codigo_documento
            recSetAuxActualizar!codigo_unidad = recSetAuxcomp!codigo_unidad  'v_UNIDAD
            recSetAuxActualizar!codigo_solicitud = recSetAuxcomp!codigo_solicitud   'V_SOLICITUD
            If IsNull(recSetAuxcomp!fecha_pago) = True Or recSetAuxcomp!fecha_pago = "01/01/1900" Then
              Fecha = Format(Date, "dd/mm/yyyy")
            Else
              Fecha = recSetAuxcomp!fecha_pago
            End If
            recSetAuxActualizar!fecha_A = CDate(Fecha) 'CDate(recSetAuxcomp!fecha_pago)
            ...
            If (recSetAuxActualizar1.BOF) And (recSetAuxActualizar1.EOF) Then
                ...
                '************* CONTABILIZA AUXILIARES CREDITO
                ...
                If recSetAuxcomp!estado_prestamo = "S" Then
                    recSetAuxActualizar1!d_montoBs = Round(recSetAuxcomp!monto_Bolivianos * 0.87, 2)
                    recSetAuxActualizar1!d_montoDl = Round(recSetAuxcomp!monto_dolares * 0.87, 2)
                    recSetAuxActualizar1!h_montoBs = Round(recSetAuxcomp!monto_Bolivianos * 0.87, 2)
                    recSetAuxActualizar1!h_montoDl = Round(recSetAuxcomp!monto_dolares * 0.87, 2)
                Else
                    recSetAuxActualizar1!d_montoBs = Round(recSetAuxcomp!monto_Bolivianos, 2)
                    recSetAuxActualizar1!d_montoDl = Round(recSetAuxcomp!monto_dolares, 2)
                    recSetAuxActualizar1!h_montoBs = Round(recSetAuxcomp!monto_Bolivianos, 2)
            ...
            If recSetAuxcomp!estado_prestamo = "S" Then
                ...
                Select Case v_Aux1
                  Case "01"
                    recSetAuxActualizar1!d_cta_larga = benef2
                    recSetAuxActualizar1!D_Des_Larga = benef2_nom
                  Case "02"
                    recSetAuxActualizar1!d_cta_larga = IIf(IsNull(recSetAuxcomp!cta_codigo), "-", recSetAuxcomp!cta_codigo)
                    recSetAuxActualizar1!D_Des_Larga = fBuscaCta(recSetAuxcomp!cta_codigo)
                  Case "08"
                ...
                Select Case v_H_Aux1
                  Case "01"
                    recSetAuxActualizar1!h_cta_larga = benef2
                    recSetAuxActualizar1!H_Des_Larga = benef2_nom
                  Case "02"
                    recSetAuxActualizar1!h_cta_larga = IIf(IsNull(recSetAuxcomp!cta_codigo), "", recSetAuxcomp!cta_codigo)
                    recSetAuxActualizar1!H_Des_Larga = fBuscaCta(recSetAuxcomp!cta_codigo)
                  Case "08"
                ...
                recSetAuxActualizar1!h_subcta2 = v_H_SubCta2
                If recSetAuxcomp!estado_prestamo = "S" Then
                    recSetAuxActualizar1!d_montoBs = Round(recSetAuxcomp!monto_Bolivianos * 0.13, 2)
                    recSetAuxActualizar1!d_montoDl = Round(recSetAuxcomp!monto_dolares * 0.13, 2)
                    recSetAuxActualizar1!h_montoBs = Round(recSetAuxcomp!monto_Bolivianos * 0.13, 2)
        ...
        Else
          ...
          recSetAuxActualizar!codigo_documento = recSetAuxcomp!codigo_documento
          recSetAuxActualizar!num_respaldo = recSetAuxcomp!Codigo_orden
          recSetAuxActualizar!codigo_unidad = v_UNIDAD
          recSetAuxActualizar!codigo_solicitud = V_SOLICITUD
          If IsNull(recSetAuxcomp!fecha_pago) = True Then
            Fecha = Format(Date, "dd/mm/yyyy")
          Else
            Fecha = recSetAuxcomp!fecha_pago
          End If
...
Wend
MsgBox "Contabilizacion exitosa...... ", vbExclamation + vbOKOnly, "CONTABILIZACION"
db.CommitTrans
    ...
    recSetPartida.CursorLocation = adUseClient
    If recSetPartida.State = 1 Then recSetPartida.Close
    'para contabilizar la nueva modalidad de pagos de la KFW(516)
'    If P_org_codigo = "516" Then
'      Call AsientoKFW_TGN(Val(P_codigo_pago), P_org_codigo, P_ges_gestion)
'    End If
Exit Sub
errorPag:
db.RollbackTrans
...
Function fBuscaBenef(nomb_1)
  '======== BUSCA EL BENEFICIARIO ========
  Dim Rs_Busca As New ADODB.Recordset
  Set Rs_Busca = New ADODB.Recordset
  If Rs_Busca.State = 1 Then Rs_Busca.Close
  Rs_Busca.Open "SELECT * From FC_BENEFICIARIO WHERE CODIGO_BENEFICIARIO = '" & nomb_1 & "'", db, adOpenKeyset, adLockReadOnly
  If Rs_Busca.RecordCount > 0 Then
    fBuscaBenef = Rs_Busca!denominacion_beneficiario
  Else
    fBuscaBenef = "-"
  End If
  If Rs_Busca.State = 1 Then Rs_Busca.Close
...
Function fBuscaCta(nomb_1)
  '======== BUSCA LA CUENTA BANCARIA ========
  Dim Rs_Busca As New ADODB.Recordset
  Set Rs_Busca = New ADODB.Recordset
  If Rs_Busca.State = 1 Then Rs_Busca.Close
  Rs_Busca.Open "SELECT * From fc_cuenta_bancaria WHERE Cta_codigo = '" & nomb_1 & "'", db, adOpenKeyset, adLockReadOnly
  If Rs_Busca.RecordCount > 0 Then
    fBuscaCta = Rs_Busca!Cta_descripcion_larga
  Else
    fBuscaCta = "-"
  End If
  If Rs_Busca.State = 1 Then Rs_Busca.Close
...
Function fBuscaOrg(orga_1)
  '======== BUSCA EL ORGANISMO FINANCIADOR ========
  Dim Rs_Busca As New ADODB.Recordset
  Set Rs_Busca = New ADODB.Recordset
  If Rs_Busca.State = 1 Then Rs_Busca.Close
  Rs_Busca.Open "SELECT * From fc_organismo_financiamiento WHERE Org_codigo = '" & orga_1 & "'", db, adOpenKeyset, adLockReadOnly
  If Rs_Busca.RecordCount > 0 Then
    fBuscaOrg = Rs_Busca!org_descripcion
  Else
    fBuscaOrg = "-"
  End If
  If Rs_Busca.State = 1 Then Rs_Busca.Close
...
Function fBuscaConv(orga_1)
  '======== BUSCA EL CODIGO DE FUENTE EN BASE AL ORGANISMO ========
  Dim Rs_Busca As New ADODB.Recordset
  Set Rs_Busca = New ADODB.Recordset
  If Rs_Busca.State = 1 Then Rs_Busca.Close
  Rs_Busca.Open "SELECT * From fc_convenios WHERE Codigo_Convenio = '" & orga_1 & "'", db, adOpenKeyset, adLockReadOnly
  If Rs_Busca.RecordCount > 0 Then
    fBuscaConv = Rs_Busca!Denominacion_Convenio
  Else
    fBuscaConv = "-"
  End If
  If Rs_Busca.State = 1 Then Rs_Busca.Close
...
'********* Para obtener en el recordset recsetAuxComp los datos necesarios para almacenar*********"
    ...
    " pago_detalle.Ges_Gestion = pagos.Ges_Gestion AND pagos.codigo_Pago= " & P_codigo_pago & " and pagos.Org_Codigo='" & P_org_codigo & "' and pago_detalle.Ges_Gestion = '" & P_ges_gestion & "'", db, adOpenDynamic, adLockOptimistic, adCmdText
...
' Datos Para co_Comprobante
            ...
            recSetAuxActualizar!ges_gestion = recSetAuxcomp!ges_gestion
            recSetAuxActualizar!num_respaldo = recSetAuxcomp!Codigo_orden
            recSetAuxActualizar!codigo_solicitud = recSetAuxcomp!Codigo_orden
            recSetAuxActualizar!codigo_documento = recSetAuxcomp!codigo_documento
            'cambio de fecha de aprobacion
            If IsNull(recSetAuxcomp!fecha_pago) = True Then
              Fecha = Format(Date, "dd/mm/yyyy")
            Else
              Fecha = recSetAuxcomp!fecha_pago
            End If
            recSetAuxActualizar!fecha_A = Fecha 'IIf(IsNull(recSetAuxcomp!fecha_pago), (Format(Date, "dd/mm/yyyy")), CDate(recSetAuxcomp!fecha_pago))
            recSetAuxActualizar!glosa = recSetAuxcomp!justificacion
...
'            End If
            ...
            Else
              Fecha = recSetAuxcomp!fecha_pago
            End If
            recSetAuxActualizar!fecha_A = Fecha 'IIf(IsNull(recSetAuxcomp!fecha_pago), Format(Date, "dd/mm/yyyy"), CDate(Format(recSetAuxcomp!fecha_pago, "dd/mm/yyyy")))
            recSetAuxActualizar!glosa = recSetAuxcomp!justificacion
            recSetAuxActualizar!usr_usuario = glusuario
            recSetAuxActualizar!fecha_registro = CDate(Format(Date, "dd/mm/yyyy"))
            recSetAuxActualizar!hora_registro = Format(Time, "hh:mm:ss")
...
'    " Pago_Detalle.Codigo_Beneficiario,pagos.Justificacion,pago_detalle.fecha_pago,pago_detalle.par_codigo,pago_detalle.Monto_Total as Monto_total,Pagos.org_Codigo,pagos.Codigo_orden,Pagos.Codigo_documento," & _
'    " pago_detalle.pro_programa, pago_detalle.pro_subprograma, pago_detalle.pro_proyecto, pago_detalle.pro_actividad, " & _
'    " pago_detalle.Monto_Dolares,pago_detalle.Tipo_Cambio,pago_detalle.estado_aprobacion From pago_detalle,pagos Where pago_detalle.codigo_Pago = pagos.codigo_Pago and pago_detalle.Org_Codigo = pagos.Org_codigo and TIPO_COMP='DAC' AND " & _
...
'                        MsgBox "NUMERO DE 1era. CUENTA DAC" & cocmCod_Comp
                rsCoCoM.AddNew
                    ...
                    rsCoCoM("fecha_registro") = CDate(Format(Date, "dd/mm/yyyy"))
                    rsCoCoM("hora_registro") = Format(Time, "hh:mm:ss")
'                    rsCoCoM!ESTADO_PRESTAMO = "n"
'                    rsCoCoM!ESTADO_ARCHIVO = "n"
'                    rsCoCoM!codigo_solicitud = "n"
'                    rsCoCoM!TIPO_MONEDA = "n"
'                    rsCoCoM!estado = "n"
'                    rsCoCoM!Origen = "n"
                rsCoCoM.Update
                ...
                If rsdiario.RecordCount > 0 Then
                    ...
                    AuxD_CtaAux2 = IIf(IsNull(rsdiario!d_ctaaux2), "", rsdiario!d_ctaaux2)
                    AuxD_CtaAux3 = IIf(IsNull(rsdiario!d_CtaAux3), "", rsdiario!d_CtaAux3)
                    AuxH_CtaAux2 = IIf(IsNull(rsdiario!h_ctaaux2), "", rsdiario!h_ctaaux2)
                    AuxH_CtaAux3 = IIf(IsNull(rsdiario!h_CtaAux3), "", rsdiario!h_CtaAux3)
                    ...
                    rsdiario!h_ctaaux2 = AuxD_CtaAux2
                    rsdiario!h_CtaAux3 = AuxD_CtaAux3
                    rsdiario!d_ctaaux2 = AuxH_CtaAux2
                    rsdiario!d_CtaAux3 = AuxH_CtaAux3
...
'JQA 24/11/01
Public Sub Cmd_contabiliza(Codigo As Integer, org As String, gestion As String)
